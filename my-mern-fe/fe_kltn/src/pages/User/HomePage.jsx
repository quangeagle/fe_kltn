  ...
  return (
    <div className="min-h-screen bg-gray-900">
        ...
        <div className="flex items-center justify-between">
          <div className="flex items-center space-x-4">
            ...
            <nav className="hidden md:flex space-x-6">
              ...
              <a href="/cart" className="text-white hover:text-purple-400 transition-colors">Giỏ Hàng</a>
              <a href="#" className="text-white hover:text-purple-400 transition-colors">Đăng Nhập</a>
              <a href="/supplier/register" className="text-white hover:text-purple-400 transition-colors">Đăng Ký</a>
            </nav>
          </div>
          <div className="flex items-center space-x-4">
      ...
      <section className="px-6 py-12 relative overflow-hidden">
        <div className="absolute top-0 left-0 w-64 h-64 bg-gradient-to-r from-pink-500 to-purple-500 rounded-full filter blur-3xl opacity-20"></div>
        <div className="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
          <div className="text-white">
            ...
            <div className="flex space-x-4">
             ...
             <div className="absolute -top-4 -left-4 w-16 h-16 bg-gradient-to-r from-pink-500 to-purple-500 rounded-full"></div>
             <div className="absolute -bottom-4 -right-4 w-12 h-12 bg-gradient-to-r from-orange-500 to-pink-500 rounded-full"></div>
             <div className="absolute top-8 -right-8 w-8 h-8 border-2 border-orange-400 rounded-full"></div>
           </div>
        </div>
      </section>

      {/* Top Artists Section (Stores) */}
      <section className="px-6 py-12">
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {Array.isArray(suppliers) && suppliers.slice(0, 4).map((supplier, index) => (
              <div key={supplier._id} className={`bg-gray-800 rounded-xl p-6 text-center ${index === 0 ? 'ring-2 ring-gradient-to-r from-yellow-400 via-orange-500 to-pink-500' : ''}`}>
      ...
      {/* Live Auctions Section (Categories) */}
        ...
        <div className="max-w-6xl mx-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {Array.isArray(categories) && categories.slice(0, 4).map((category, index) => (
              <div key={category._id} className="bg-white rounded-xl p-6">
                <div className={`w-full h-32 rounded-lg mb-4 flex items-center justify-center ${
                  index === 0 ? 'bg-pink-100' : 
                  index === 1 ? 'bg-purple-100' : 
                  index === 2 ? 'bg-pink-100' : 'bg-teal-100'
                }`}>
                  <div className="w-16 h-16 bg-gradient-to-r from-purple-500 to-orange-500 rounded-lg flex items-center justify-center">
                    <span className="text-white font-bold text-lg">{category.name?.charAt(0) || 'C'}</span>
                  </div>
                </div>
                <div className="flex items-center justify-between mb-2">
                  <span className="text-pink-500 text-sm">❤️ {(Math.random() * 5 + 0.5).toFixed(1)}K</span>
                </div>
                <h3 className="text-gray-900 font-semibold">{category.name || 'Category'}</h3>
              </div>
            ))}
          </div>
        </div>
      ...
      {/* Today's Picks Section (Products) */}
      <section className="px-6 py-12 border-t border-gray-700">
        <div className="max-w-6xl mx-auto">
          <div className="flex items-center justify-between mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {Array.isArray(products) && products.slice(0, 8).map((product, index) => (
              <div key={product._id} className="bg-gray-800 rounded-xl p-6 border border-gray-700">
...
/>  
                  <div className="absolute top-2 right-2 flex items-center space-x-1 bg-black bg-opacity-50 text-white px-2 py-1 rounded text-sm">
                    <span>❤️</span>
                ...
                <div className="flex items-center justify-between mb-4">
                  <div className="flex items-center space-x-2">
                    ...
                    <span className="text-gray-400 text-sm">@{product.supplier?.storeName?.slice(0, 10)}...</span>
                  </div>
                  <span className="text-white font-semibold">{product.price?.toLocaleString('vi-VN')}₫</span>
                </div>
                <div className="flex space-x-2">
                  <button 
